    ...
    <div className="problem-header">
      ...
      <span className="problem-stats">
        <p>
          ...
          <span
            className={`${
              QUESTION_DIFFICULTY[problem.p_difficulty].style
...
const Arena = () => {
  ...
  const { sendMessage: questionExecCode, messages: questionResult } =
    useWebSocket<ArenaCodeReq, ArenaCodeRes>("/arena/code_execute");
  ...
  useEffect(() => {
    if (router.query.id) {
      ProblemAPI.problemGetById({ p_id: router.query.id as string }).then(
        (data) => {
